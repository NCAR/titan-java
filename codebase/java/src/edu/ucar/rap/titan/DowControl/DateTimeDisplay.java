                                                       ...
                                                       
{
  ...
  public static int LATEST_HIGH = 1;
  public static int LATEST_LOW = 2;
  public static int START_HIGH = 3;
  public static int START_LOW = 4;
  ...
  public DateTimeDisplay(JFrame frame,
    ...
    } else if (_role == LATEST_HIGH) {
      setBorder(CustomBorder.createTop(frame, "Latest time high", 1));
    } else if (_role == LATEST_LOW) {
      setBorder(CustomBorder.createTop(frame, "Latest time low", 1));
    } else if (_role == START_HIGH) {
      setBorder(CustomBorder.createTop(frame, "Start time high", 1));
    } else if (_role == START_LOW) {
      setBorder(CustomBorder.createTop(frame, "Start time low", 1));
    }
  ...
  public void redraw() {
    ...
    if (_role == GPS) {
      ...
      dtime = _status.pentekHigh.latest_time;
    } else if (_role == LATEST_LOW) {
      dtime = _status.pentekLow.latest_time;
    } else if (_role == START_HIGH) {
      dtime = _status.pentekHigh.start_time;
    } else if (_role == START_LOW) {
      dtime = _status.pentekLow.start_time;
    }
