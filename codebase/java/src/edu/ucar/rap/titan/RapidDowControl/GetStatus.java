...
public class GetStatus extends JPanel {
  ...
  private void _loadStatusFromXmlDoc(Document doc,
    ...
    double val;
    Element root = doc.getRootElement();
    if (root.getName().equals("RapiDrxReply")) {
      Iterator itRoot = root.getChildren().iterator();
      while (itRoot.hasNext()) {
        ...
        if (elc.getName().equals("Status")) {
          ...
          while (itc.hasNext()) {
            ...
            if (name.equals("Pentek1")) {
              _loadPentekStatus(status.pentek1, el);
            } else if (name.equals("Pentek2")) {
              _loadPentekStatus(status.pentek2, el);
            }
            ...
            if (name.equals("pulse_width_sec")) {
              status.pulse_width_sec = _getDoubleFromStr(name, text);
            }
            if (name.equals("tx_envelope_width_sec")) {
              status.tx_envelope_width_sec = _getDoubleFromStr(name, text);
            }
            if (name.equals("duty_cycle")) {
              status.duty_cycle = _getDoubleFromStr(name, text);
            }
                                 ...
                                 Element pentekRoot) 
  {
    ...
    while (itp.hasNext()) {
      ...
      if (name.equals("freq_channel_0")) {
        pentekData.chan_0_freq_hz = _getDoubleFromStr(name, text);
      }
      if (name.equals("freq_channel_1")) {
        pentekData.chan_1_freq_hz = _getDoubleFromStr(name, text);
      }
      if (name.equals("freq_channel_2")) {
        pentekData.chan_2_freq_hz = _getDoubleFromStr(name, text);
      }
